...
/// `bitrouter ledger` — read the most recent settled requests from the local
/// metering DB for the unified spend + receipts view (PRD §8.6). Connects to
/// `db_url` and runs migrations first, so the receipt columns exist even on a
/// DB created before they were added.
pub async fn list_ledger(db_url: &str, limit: u64) -> Result<Vec<crate::metering::LedgerEntry>> {
    let db = crate::db::connect(db_url)
        .await
        .with_context(|| format!("connecting to database {db_url}"))?;
    crate::db::run_migrations(&db)
        .await
        .context("running database migrations")?;
    crate::metering::MeteringStore::new(db)
        .list_recent(limit)
        .await

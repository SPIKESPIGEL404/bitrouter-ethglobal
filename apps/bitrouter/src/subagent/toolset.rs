...
impl SpawnSubagentToolset {
    ...
    async fn run_spawn(&self, args: SpawnArgs) -> Result<ToolResultOutput> {
        ...
        if !self.config.models.iter().any(|m| m.id() == args.model) {
            let allowed: Vec<&str> = self.config.models.iter().map(|m| m.id()).collect();
    ...
    fn tool_schema(&self) -> Tool {
        let ids: Vec<&str> = self.config.models.iter().map(|m| m.id()).collect();
        let catalog = self
            .config
            .models
            .iter()
            .map(|m| {
                ...
                let price = m.price().map(|p| format!(" price={p}")).unwrap_or_default();
                format!("- {}{caps}{price}", m.id())
            })
            .collect::<Vec<_>>()
            .join("\n");
        Tool::Function {
            ...
            description: Some(format!(
                "Spawn a budget-capped subagent on a chosen model to perform a coding task. \
                 Pick `model` from the catalog below by cost / capability — the cheapest model \
                 whose capabilities cover the task. The subagent's spend is hard-capped; address \
            ...
            parameters: serde_json::json!({
                ...
                "properties": {
                    "model": {
                        "type": "string",
                        "enum": ids,
                        "description": "Model id for the subagent (must be in the catalog)."
                    },
                    "budget_micro_usd": {
                        "type": "integer",
                        "minimum": 1,
...
mod tests {
    ...
    #[tokio::test]
    async fn tool_schema_surfaces_the_catalog() {
        use bitrouter_sdk::language_model::server_tools::config::ModelChoice;
        let db = crate::db::connect("sqlite::memory:").await.unwrap();
        crate::db::run_migrations(&db).await.unwrap();
        let ts = SpawnSubagentToolset::new(
            ...
            SpawnSubagentConfig {
                models: vec![
                    ...
                    ModelChoice::Entry {
                        id: "smart".into(),
                        caps: vec!["long-context".into()],
                        price: Some("0.006".into()),
                    },
                ],
                ..Default::default()
            },
        ...
        let tools = ts.list_tools(&ctx()).await.unwrap();
        let desc = match &tools[0] {
            Tool::Function { description, .. } => description.clone().unwrap_or_default(),
            _ => panic!("expected a function tool"),
        };
        // The orchestrator chooses by cost/capability, so the catalog (id +
        // caps + price) must reach it through the tool description.
        assert!(desc.contains("smart"), "catalog id should appear: {desc}");
        assert!(desc.contains("long-context"), "caps should appear");
        assert!(desc.contains("0.006"), "price should appear");
    }
    ...
    async fn registers_policy_and_mints_key_before_spawn() {
        ...
        assert!(!store.is_empty(), "policy registered before spawn");
    }
}

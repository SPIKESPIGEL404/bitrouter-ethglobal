...
Cloud's managed provider network means you make model requests without setting up upstream accounts or storing API keys. Today this is [Managed Models](/docs/get-started/managed-models) — one account, requests billed per token, open models served at a discount below official pricing. Managed Tools and Agents are on the roadmap.
...
- **Using Cloud managed models** — see [Managed Models](/docs/get-started/managed-models) for the model catalog, pricing, and how to make your first managed request.

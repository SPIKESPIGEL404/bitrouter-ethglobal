...
server_tools:
  spawn_subagent:
    ...
    models:                                # catalog; a call naming a model outside it is rejected
      - "bitrouter/z-ai/glm-5.1"           # bare id, or a {id, caps, price} entry:
      - id: "bitrouter/kimi-k2.6"
        caps: ["text", "long-context"]     # capability tags — informs the orchestrator's choice
        price: "0.006"                     # indicative price (display only)
```

Bare ids and `{ id, caps, price }` entries can be mixed. The `caps`/`price`
metadata is surfaced in the tool description so the orchestrator can pick the
cheapest model whose capabilities cover the task (PRD §8.3).
